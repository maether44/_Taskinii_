 ...
 *
 * Embedding pipeline for Yara's vector RAG.
 *
 * Reads a user's data from every relevant table, chunks it into embeddable
 * text blocks, generates 384-dim embeddings via HF Inference API (gte-small),
 * and upserts them into the rag_chunks table.
 *
 * Invocation modes:
 *   1. Single user:  POST { "userId": "<uuid>" }
 *      → Re-embeds all data for that user. Called after data changes (meal log,
 *        workout complete, profile update, etc.).
 *
 *   2. Incremental:  POST { "userId": "<uuid>", "chunkTypes": ["meal_log","workout_session"] }
 *      → Re-embeds only the specified chunk types. Cheaper for targeted refreshes.
 *
 *   3. All users:    POST { "all": true, "adminKey": "<secret>" }
 *      → Batch re-embed for every user. For initial backfill or scheduled cron.
 */
...
async function fetchRecentMeals(sb: any, userId: string) {
  const { data } = await sb.rpc('get_user_nutrition_summary', { p_user_id: userId })
  let parsed = data
  if (typeof data === 'string') {
    try { parsed = JSON.parse(data) } catch { return [] }
  }
  return Array.isArray(parsed?.recent_meals) ? parsed.recent_meals : []
}
...
async function embedUserData(
  ...
  // Nutrition summary
  if (targetTypes.has('nutrition_summary')) {
    try {
      const nutrition = await fetchNutritionSummary(sb, userId)
      if (nutrition) track(chunkNutritionSummary(nutrition))
    } catch (e: any) {
  ...
  if (targetTypes.has('meal_log')) {
    try {
      const meals = await fetchRecentMeals(sb, userId)
  ...
  // Batch embed + upsert (HF API supports batching, embeddings.ts handles it)
  // Process in batches of 16 to stay within HF rate limits
  const BATCH_SIZE = 16
  let totalUpserted = 0
  ...
  // Delete stale chunks for each type we refreshed
  let totalCleaned = 0
