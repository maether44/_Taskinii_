...
export function useWorkouts(userId) {
  ...
  const finishSession = useCallback(
    async (sessionId, { caloriesBurned, notes }) => {
      if (!sessionId) return;
      await supabase
        .from("workout_sessions")
        .update({
  ...
  const logExercise = useCallback(
    async (sessionId, { exerciseId, sets, reps, weightKg, durationSecs, postureScore }) => {
  ...
  const history = (sessions || []).map((s) => {
    ...
    const durationM = start && end ? Math.round((end - start) / 60000) : 0;
