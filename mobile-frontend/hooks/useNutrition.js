...
export function useNutrition() {
  ...
  // Log water — guest mode uses local state, auth mode uses Supabase
  // Also schedules/cancels 5 PM hydration reminder based on daily goal
  const logWater = useCallback(
    async (mlDelta) => {
      ...
      if (!userId) {
        ...
        newMl = Math.max(0, waterMl + mlDelta);
        setWaterMl(newMl);
      } else {
        // Auth mode: update Supabase
        try {
          const { data: existing } = await supabase
            .from("daily_activity")
            .select("*")
            ...
            .eq("date", TODAY)
            .single();
          newMl = Math.max(0, (existing?.water_ml || 0) + mlDelta);
          if (existing) {
            await supabase
              .from("daily_activity")
              .update({ water_ml: newMl })
              .eq("id", existing.id);
          } else {
            await supabase
              .from("daily_activity")
  ...
  const totals = allLogs.reduce(
    (acc, log) => {
      ...
      return {
        calories:
          acc.calories + Math.round((food.calories_per_100g || 0) * scale),
  ...
  const mealsBySlot = ["breakfast", "lunch", "dinner", "snacks"].reduce(
    (acc, slot) => {
      ...
      acc[slot] = {
        ...
        cal: slotLogs.reduce((s, l) => {
          ...
          return food
            ? s +
                Math.round(
                  ((food.calories_per_100g || 0) * (l.quantity_grams || 100)) /
                    100,
                )
            : s;
        }, 0),
