...
const GOAL_LABELS = {
  ...
  gain_muscle:'Build Muscle 💪', muscle_gain:'Build Muscle 💪',
  maintain:'Stay Fit ⚖️', gain_weight:'Gain Weight 🍽️', build_habits:'Build Habits 🧠',
};
...
const EDITABLE_GOALS = [
  { id:'lose_fat', label:'Lose Fat' }, { id:'gain_muscle', label:'Build Muscle' },
  { id:'gain_weight', label:'Gain Weight' }, { id:'maintain', label:'Stay Fit' },
...
export default function Profile({ replayTour }) {
  ...
  const [editForm, setEditForm] = useState({
    full_name:'', gender:'male', height_cm:'', weight_kg:'',
  ...
  const loadProfileData = useCallback(async () => {
    ...
    try {
      ...
      const [{ data: cal }, profileResult] = await Promise.all([
        ...
        supabase
          .from('profiles')
          .select('full_name, bio, city, country, date_of_birth, gender, height_cm, weight_kg, goal, activity_level, target_weight_kg, avatar_url')
          .eq('id', authUser.id)
          .single(),
      ]);
      ...
      if (profileResult?.error && /avatar_url/i.test(profileResult.error.message || '')) {
        const { data: fallbackProf, error: fallbackError } = await supabase
          .from('profiles')
          .select('full_name, bio, city, country, date_of_birth, gender, height_cm, weight_kg, goal, activity_level, target_weight_kg, xp_current, level')
          .eq('id', authUser.id)
          .single();
      ...
      try {
        ...
        setXpInfo(!xpError && xpData ? xpData : { level:1, xp_current:0, xp_total:0, xp_needed:100 });
      } catch { setXpInfo({ level:1, xp_current:0, xp_total:0, xp_needed:100 }); }
  ...
  const calTarget = calorieTarget || (
    goal === 'lose_fat' ? tdee - 400 : goal === 'gain_muscle' ? tdee + 200 :
    goal === 'gain_weight' ? tdee + 400 : tdee
  );
  ...
  const goalNote  = goal === 'lose_fat' ? '400 kcal deficit for steady fat loss (~0.4 kg/week)'
    : goal === 'gain_muscle' ? '200 kcal surplus for lean muscle building'
    : 'Maintenance calories — staying fit and healthy';
  const goalDescription = goal === 'gain_weight' ? '400 kcal surplus to support healthy weight gain' : goalNote;
  ...
  const saveProfileChanges = async () => {
    ...
    if (!nextHeight || !nextWeight) { Alert.alert('Missing info', 'Height and weight are required.'); return; }

    setSavingProfile(true);
    try {
      const { error: profileError } = await supabase.from('profiles').update({
        full_name: nextName, gender: editForm.gender,
        height_cm: nextHeight, weight_kg: nextWeight,
        target_weight_kg: nextTargetWeight, goal: nextGoal,
        activity_level: editForm.activity_level, updated_at: new Date().toISOString(),
      }).eq('id', authUser.id);
      ...
      const nextCalories = nextGoal === 'lose_fat' ? nextTdee - 400 : nextGoal === 'gain_muscle' ? nextTdee + 200 : nextGoal === 'gain_weight' ? nextTdee + 400 : nextTdee;
      ...
      const { error: targetsError } = await supabase.from('calorie_targets').insert({
        user_id: authUser.id, daily_calories: nextCalories,
        protein_target: nextProtein || macros.protein_target,
        carbs_target: macros.carbs_target, fat_target: macros.fat_target,
        effective_from: new Date().toISOString().split('T')[0],
      });
      if (targetsError) throw targetsError;

      setProfile(prev => ({ ...prev, full_name: nextName, gender: editForm.gender, height_cm: nextHeight, weight_kg: nextWeight, target_weight_kg: nextTargetWeight, goal: nextGoal, activity_level: editForm.activity_level }));
  ...
  // ── Password Reset ─────────────────────────────────────────────────────────
  const openPwModal = () => { setPwForm({ newPassword:'', confirmPassword:'' }); setPwModalVisible(true); };

  const saveNewPassword = async () => {
    if (!pwForm.newPassword || pwForm.newPassword.length < 6) { Alert.alert('Too short', 'Password must be at least 6 characters.'); return; }
    if (pwForm.newPassword !== pwForm.confirmPassword) { Alert.alert('Mismatch', 'Passwords do not match.'); return; }
    setSavingPw(true);
    try {
  ...
  const pickProfilePhoto = async () => {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from(AVATAR_BUCKET).upload(path, blob, { contentType: asset.mimeType || 'image/jpeg' });
      if (uploadError) throw uploadError;

      const { error: profileError } = await supabase.from('profiles').update({ avatar_url: path, updated_at: new Date().toISOString() }).eq('id', authUser.id);
      if (profileError) throw profileError;
  ...
  return (
    <View style={s.root}>
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.profileCard}>
          ...
          <TouchableOpacity style={s.avatarWrap} onPress={pickProfilePhoto} activeOpacity={0.85}>
            <View style={s.avatarCircle}>
              ...
              {uploadingPhoto && <View style={s.avatarOverlay}><ActivityIndicator color="#fff" /></View>}
            </View>
          ...
          <View style={s.profileInfo}>
            <View style={s.profileNameRow}>
              ...
              <TouchableOpacity style={s.editProfileBtn} onPress={() => navigation.navigate('EditProfile')} activeOpacity={0.85}>
                <Text style={s.editProfileBtnTxt}>Edit profile</Text>
              </TouchableOpacity>
            </View>
            {profile?.bio ? <Text style={s.profileBio}>{profile.bio}</Text> : null}
            {(profile?.city || profile?.country) ? (
              <Text style={s.profileLocation}>📍 {[profile.city, profile.country].filter(Boolean).join(', ')}</Text>
            ) : null}
        ...
        <TouchableOpacity style={s.historyBtn} onPress={() => navigation.navigate('WorkoutHistory')} activeOpacity={0.85}>
          <View style={s.historyBtnLeft}>
            <View style={s.historyBtnIcon}><Text style={{ fontSize:20 }}>🏋️</Text></View>
            <View>
        ...
        <View style={s.card}>
          ...
          <View style={s.targetNote}><Text style={s.targetNoteTxt}>{goalDescription}</Text></View>
        </View>
        ...
        <View style={s.card}>
          ...
          {achievements.length > 6 && (
            <TouchableOpacity style={s.seeMoreBtn} onPress={() => setShowAllAchievements(!showAllAchievements)} activeOpacity={0.8}>
              <Text style={s.seeMoreTxt}>{showAllAchievements ? 'Show Less' : `See ${achievements.length - 6} More`}</Text>
            </TouchableOpacity>
          )}
        </View>
        ...
        <View style={s.card}>
          ...
          {[
            ...
            <View key={i} style={[s.settingRow, i < 2 && s.settingRowBorder]}>
              ...
              <Switch value={n.value} onValueChange={n.set} trackColor={{ false:C.border, true:C.purple+'80' }} thumbColor={n.value ? C.purple : C.sub} />
            </View>
          ))}
        </View>
      ...
      <Modal visible={editVisible} transparent animationType="slide" onRequestClose={() => !savingProfile && setEditVisible(false)}>
        <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
          <View style={s.modalBackdrop}>
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={s.modalKeyboardWrap}>
              <TouchableWithoutFeedback>
                <View style={s.modalCard}>
                  <View style={s.modalHandle} />
                  <ScrollView contentContainerStyle={s.modalScrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                    <Text style={s.modalTitle}>Edit Body Stats</Text>

                    <Text style={s.modalLabel}>Goal</Text>
                    <View style={s.chipWrap}>
                      {EDITABLE_GOALS.map(option => (
                        <Pressable key={option.id} style={[s.chip, editForm.goal === option.id && s.chipActive]} onPress={() => updateEditField('goal', option.id)}>
                          <Text style={[s.chipText, editForm.goal === option.id && s.chipTextActive]}>{option.label}</Text>
                        </Pressable>
                      ))}
                    </View>
                    ...
                    <View style={s.chipWrap}>
                      {EDITABLE_GENDERS.map(option => (
                        <Pressable key={option} style={[s.chip, editForm.gender === option && s.chipActive]} onPress={() => updateEditField('gender', option)}>
                          <Text style={[s.chipText, editForm.gender === option && s.chipTextActive]}>{option.charAt(0).toUpperCase() + option.slice(1)}</Text>
                        </Pressable>
                      ))}
                    </View>

                    <Text style={s.modalLabel}>Height (cm)</Text>
                    <TextInput style={s.modalInput} value={editForm.height_cm} onChangeText={v => updateEditField('height_cm', v)} keyboardType="numeric" returnKeyType="done" blurOnSubmit onSubmitEditing={Keyboard.dismiss} placeholder="175" placeholderTextColor={C.sub} />

                    <Text style={s.modalLabel}>Weight (kg)</Text>
                    <TextInput style={s.modalInput} value={editForm.weight_kg} onChangeText={v => updateEditField('weight_kg', v)} keyboardType="numeric" returnKeyType="done" blurOnSubmit onSubmitEditing={Keyboard.dismiss} placeholder="75" placeholderTextColor={C.sub} />

                    <Text style={s.modalLabel}>Target Weight (kg)</Text>
                    <TextInput style={s.modalInput} value={editForm.target_weight_kg} onChangeText={v => updateEditField('target_weight_kg', v)} keyboardType="numeric" returnKeyType="done" blurOnSubmit onSubmitEditing={Keyboard.dismiss} placeholder="Optional" placeholderTextColor={C.sub} />

                    <Text style={s.modalLabel}>Activity Level</Text>
                    <View style={s.chipWrap}>
                      {Object.entries(ACTIVITY_LABELS).map(([id, label]) => (
                        <Pressable key={id} style={[s.chip, editForm.activity_level === id && s.chipActive]} onPress={() => updateEditField('activity_level', id)}>
                          <Text style={[s.chipText, editForm.activity_level === id && s.chipTextActive]}>{label}</Text>
                        </Pressable>
                      ))}
                    </View>
                  </ScrollView>
                  ...
                  <View style={s.modalActions}>
                    <Pressable style={[s.modalBtn, s.modalBtnSecondary]} onPress={() => !savingProfile && setEditVisible(false)}>
                      <Text style={s.modalBtnSecondaryText}>Cancel</Text>
                    </Pressable>
                    <Pressable style={[s.modalBtn, s.modalBtnPrimary, savingProfile && s.modalBtnDisabled]} onPress={saveProfileChanges} disabled={savingProfile}>
                      <Text style={s.modalBtnPrimaryText}>{savingProfile ? 'Saving...' : 'Save'}</Text>
                    </Pressable>
                  </View>
                </View>
              </TouchableWithoutFeedback>
      ...
      {/* ── Achievement Popup ── */}
      <Modal visible={!!achievementPopup} transparent animationType="fade" onRequestClose={() => setAchievementPopup(null)}>
...
const s = StyleSheet.create({
  ...
  // Profile card
  profileCard:       { backgroundColor:C.card, borderRadius:20, padding:18, marginBottom:14, borderWidth:1, borderColor:C.border, flexDirection:'row', alignItems:'flex-start', gap:16 },
  avatarWrap:        { position:'relative', alignItems:'center' },
  avatarCircle:      { width:80, height:80, borderRadius:40, backgroundColor:C.purple, alignItems:'center', justifyContent:'center', borderWidth:3, borderColor:C.accent, overflow:'hidden' },
  ...
  avatarEditBadge:   { position:'absolute', bottom:-6, backgroundColor:C.lime, borderRadius:999, paddingHorizontal:8, paddingVertical:3, borderWidth:2, borderColor:C.card },
  avatarEditBadgeTxt:{ color:'#161230', fontSize:10, fontWeight:'800' },

  profileInfo:    { flex:1 },
  profileNameRow: { flexDirection:'row', alignItems:'center', justifyContent:'space-between', marginBottom:4 },
  ...
  editProfileBtn:    { flexDirection:'row', alignItems:'center', gap:4, backgroundColor:C.purple+'20', borderRadius:8, paddingHorizontal:8, paddingVertical:5, borderWidth:1, borderColor:C.purple+'50' },
  editProfileBtnTxt: { color:C.accent, fontSize:11, fontWeight:'700' },

  // History button
  historyBtn:      { backgroundColor:C.card, borderRadius:20, padding:16, marginBottom:14, borderWidth:1, borderColor:C.border, flexDirection:'row', alignItems:'center', justifyContent:'space-between' },
  historyBtnLeft:  { flexDirection:'row', alignItems:'center', gap:12 },
  historyBtnIcon:  { width:44, height:44, borderRadius:12, backgroundColor:C.lime+'15', alignItems:'center', justifyContent:'center' },
  ...
  changePwBtn: { backgroundColor:C.card, borderRadius:16, paddingVertical:16, alignItems:'center', borderWidth:1, borderColor:C.border, marginBottom:10 },
  ...
  modalCard:          { backgroundColor:C.card, borderTopLeftRadius:24, borderTopRightRadius:24, paddingTop:10, paddingHorizontal:20, paddingBottom:32, maxHeight:'88%', borderTopWidth:1, borderColor:C.border },
