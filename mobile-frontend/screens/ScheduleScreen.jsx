...
function RingProgress({ pct, size = 72, stroke = 6, color, label, actual, target, unit }) {
  ...
  return (
    <View style={{ alignItems: "center", gap: 5 }}>
      <View style={{ width: size, height: size, alignItems: "center", justifyContent: "center" }}>
        ...
        <View
          style={{
            ...
            backgroundColor: done ? color + "22" : color + "14",
            alignItems: "center",
            justifyContent: "center",
          }}
        >
          <Text style={{ color: done ? color : "#F4F0FF", fontSize: 11, fontWeight: "800" }}>
            {actual ?? "—"}
          </Text>
      ...
      <Text style={{ color: "#3D3560", fontSize: 9 }}>
        /{target}
        {unit}
      </Text>
    </View>
    ...
    <Animated.View style={[ex.row, { opacity: fade, transform: [{ translateY: slide }] }]}>
      ...
      <View style={{ flex: 1 }}>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
          <Text
            style={[ex.name, checked && { textDecorationLine: "line-through", color: "#4A4268" }]}
          >
            {exercise.name}
          </Text>
          {carriedOver && (
...
export default function ScheduleScreen() {
  ...
  const fmtSteps = (v) =>
    v != null ? (v >= 1000 ? `${(v / 1000).toFixed(1)}k` : String(v)) : null;
        ...
        <>
          ...
          >
            {schedule.days.map((d, i) => {
              ...
              const isMissed =
                i > 0 && !dComp.done && !d.is_rest && d.exercises?.length > 0 && dDate < todayDate;
                ...
                >
                  ...
                  {dComp.done ? (
                    ...
                    <Text
                      style={[
                        s.dayTypeTxt,
                        { color: isMissed ? "#FF6B6B" : getWorkoutColor(d.workout_type) },
                      ]}
                    >
          ...
          >
            ...
            <View style={s.dayTitleRow}>
              <View
                style={[s.dayTitleDot, { backgroundColor: isDayDone ? "#C6FF33" : workoutColor }]}
              />
            ...
            {yesterdayWasMissed && (
              <View style={s.missedBanner}>
                <Text style={s.missedTitle}>
                  ⚠️ Yesterday's {yesterdayPlan?.workout_type} was missed
                </Text>
                <Text style={s.missedSub}>
                  {carriedExercises.length} uncompleted exercise
                  {carriedExercises.length !== 1 ? "s" : ""} added to today
                </Text>
              </View>
            )}
            ...
            {hasHealthCarryover && (
              <View style={s.carryoverBanner}>
                <Text style={s.carryoverTxt}>
                  ↑ Sleep/steps/water targets adjusted from yesterday
                </Text>
              </View>
            )}
            ...
            <View style={s.sectionCard}>
              ...
              <View style={s.ringsRow}>
                ...
                <RingProgress
                  ...
                  color="#C6FF33"
                  label="Steps"
                  actual={fmtSteps(stepsActual) ?? "—"}
                  target={stepsTarget >= 1000 ? `${(stepsTarget / 1000).toFixed(0)}k` : stepsTarget}
                  unit="steps"
                />
                <RingProgress
                  pct={waterPct}
              ...
              <View style={s.sectionCard}>
                <View style={s.sectionTitleRow}>
                  ...
                  <View
                    style={[
                      s.badge,
                      { backgroundColor: workoutColor + "22", borderColor: workoutColor + "55" },
                    ]}
                  >
                    ...
                    >
                      <Text style={[s.completeDayTxt, !allChecked && { color: "#4A4268" }]}>
                        {allChecked
                          ...
                          : `Complete all ${allExercises.length - checkedCount} remaining exercises first`}
                      </Text>
                    </TouchableOpacity>
                  ))}
              </View>
            ...
            {day.is_rest && (
              <View style={[s.sectionCard, s.restCard]}>
                ...
                <Text style={s.restSub}>
                  {day.rest_note ||
                    "Focus on sleep, hydration, and light movement like walking or stretching."}
                </Text>
              </View>
            )}
            ...
            {day.meals?.length > 0 && (
              <View style={s.sectionCard}>
                <View style={s.sectionTitleRow}>
                  ...
                  {totalCals > 0 && (
                    <View
                      style={[s.badge, { backgroundColor: "#C6FF3322", borderColor: "#C6FF3355" }]}
                    >
...
const s = StyleSheet.create({
  ...
  completedBadge: {
    backgroundColor: "#C6FF3318",
    borderRadius: 14,
    paddingVertical: 12,
    alignItems: "center",
    borderWidth: 1,
    borderColor: "#C6FF3344",
  },
