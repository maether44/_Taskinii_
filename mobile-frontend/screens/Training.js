...
// ── Estimate duration & kcal from circuit exercises ─────────
function estimateCircuit(exercises) {
  ...
  const minutes = Math.round(totalSets * 2.5 + exercises.length * 1);
  const kcal = Math.round(totalSets * 8 + exercises.length * 12);
  return { duration: `${minutes} min`, kcal: `${kcal} kcal` };
}
        ...
        <Pressable onPress={() => {}}>
          <Reanimated.View style={[s.modalCard, cardStyle]}>
            ...
            >
              ...
              {/* ── SMART PIVOT (hidden for home exercises with no alternatives) ── */}
              {machine.alternatives && machine.alternatives.length > 0 && (
                <View style={s.modalSection}>
                  <TouchableOpacity
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      (todaySessions ?? []).forEach((sess) => {
        ...
        (circuit || []).forEach((ex) => {
          if (raw.includes(ex.key)) doneKeys.add(ex.key);
        });
      });
      ...
      >
        ...
        <Reanimated.View entering={FadeInDown.delay(50).springify()}>
          ...
          >
            ...
            {todayPlan.map((ex, i) => {
              const done = completedKeys.has(ex.key);
              return (
                <TouchableOpacity
                  key={i}
                  style={[
                    s.circuitRow,
                    i === todayPlan.length - 1 && { borderBottomWidth: 0 },
                  ...
                  activeOpacity={0.7}
                >
                  <View style={[s.circuitNumCircle, done && s.circuitNumDone]}>
                    {done ? (
            ...
            {/* Start CTA */}
            {(() => {
              const doneCount = todayPlan.filter((ex) => completedKeys.has(ex.key)).length;
              const allDone = todayPlan.length > 0 && doneCount === todayPlan.length;
              const nextEx = todayPlan.find((ex) => !completedKeys.has(ex.key));
              return (
                <TouchableOpacity
        ...
        <Reanimated.View entering={FadeInDown.delay(190).springify()}>
          ...
          >
            {(selectedMuscle
              ? COMBINED_EQUIPMENT.filter(
                  (m) =>
                    (m.muscles || []).some(
                      (mu) => mu.toLowerCase() === selectedMuscle.toLowerCase(),
                    ) || m.primaryMuscle.toLowerCase().includes(selectedMuscle.toLowerCase()),
                )
              : COMBINED_EQUIPMENT
            ).map((item) => (
              <MachineCard key={item.id} machine={item} onPress={(m) => openMachineModal(m)} />
            ))}
          </ScrollView>
        </Reanimated.View>
        ...
        <Reanimated.View entering={FadeInDown.delay(230).springify()}>
          ...
          {aiPlan ? (
            <View style={[s.glassCard, SHADOW, { padding: 18 }]}>
              ...
              {aiPlanError && (
                <View style={s.aiPlanError}>
                  <Ionicons name="alert-circle-outline" size={14} color="#FF6B6B" />
                  <Text style={s.aiPlanErrorTxt}>{aiPlanError}</Text>
                </View>
              )}
            </View>
          ) : (
            ...
            >
              ...
              {aiPlanError && (
                <View style={[s.aiPlanError, { marginTop: 12 }]}>
                  <Ionicons name="alert-circle-outline" size={14} color="#FF6B6B" />
                  <Text style={s.aiPlanErrorTxt}>{aiPlanError}</Text>
                </View>
              )}
