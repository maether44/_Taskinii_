// screens/Training.js — mock data replaced with static defaults
...
const EXERCISES = [
  ...
  { id:'tricep_push',  name:'Tricep Pushdown',   muscle:'Triceps',  equipment:'Cable' },
  { id:'squat',        name:'Squat',             muscle:'Quads',    equipment:'Barbell' },
  { id:'rdl',          name:'Romanian Deadlift', muscle:'Hamstrings',equipment:'Barbell' },
  { id:'leg_press',    name:'Leg Press',         muscle:'Quads',    equipment:'Machine' },
  { id:'plank',        name:'Plank',             muscle:'Core',     equipment:'Bodyweight' },
  { id:'run',          name:'Treadmill Run',     muscle:'Cardio',   equipment:'Treadmill' },
];

const PLAN = {
  name: 'Fat Loss',
  days: [
    { name:'Upper Body', estimatedMinutes:45, estimatedCalories:380, exercises:[
    ...
    { name:'Lower Body', estimatedMinutes:50, estimatedCalories:420, exercises:[
      {id:'squat',sets:4,reps:8,restSec:120},
      {id:'rdl',sets:3,reps:10,restSec:90},
      {id:'leg_press',sets:3,reps:12,restSec:60},
    ]},
    { name:'Rest Day', estimatedMinutes:0, estimatedCalories:0, exercises:[] },
    { name:'Upper Body', estimatedMinutes:45, estimatedCalories:380, exercises:[
      {id:'bench_press',sets:3,reps:10,restSec:90},
      {id:'row',sets:3,reps:12,restSec:60},
      {id:'ohp',sets:3,reps:10,restSec:60},
    ]},
    { name:'Cardio', estimatedMinutes:30, estimatedCalories:300, exercises:[
      {id:'run',duration:'30 min',restSec:0},
      {id:'plank',duration:'3×60s',restSec:30},
    ]},
    { name:'Full Body', estimatedMinutes:55, estimatedCalories:450, exercises:[
      {id:'squat',sets:3,reps:10,restSec:90},
      {id:'bench_press',sets:3,reps:10,restSec:90},
      {id:'row',sets:3,reps:12,restSec:60},
    ]},
    { name:'Rest Day', estimatedMinutes:0, estimatedCalories:0, exercises:[] },
...
const WORKOUT_HISTORY = [
  { name:'Upper Body', date:'Yesterday',    completed:true,  duration:44, calories:375 },
  { name:'Lower Body', date:'2 days ago',   completed:true,  duration:51, calories:430 },
  { name:'Rest Day',   date:'3 days ago',   completed:false, duration:0,  calories:0   },
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.weekRow}>
          {DAYS.map((d, i) => {
            ...
            return (
              ...
              >
                <Text style={[s.dayLabel, active && s.dayLabelActive, today && !active && { color:C.lime }]}>{d}</Text>
                <View style={[s.dayDot, rest ? s.dayDotRest : (active ? s.dayDotActive : s.dayDotWork)]} />
              </TouchableOpacity>
            );
          })}
        </View>
        ...
        <View style={s.card}>
          <View style={s.workoutCardTop}>
            <View>
              <Text style={s.workoutCardDay}>{isToday ? 'TODAY' : DAYS[selected].toUpperCase()}</Text>
            ...
            {!isRestDay && (
              <View style={s.workoutMeta}>
                {[
                  ...
                  { v: selectedWorkout.estimatedCalories + ' kcal', icon:'🔥' },
                  { v: selectedWorkout.exercises.length + ' exercises', icon:'💪' },
                ].map((m,i) => (
                  <View key={i} style={s.metaPill}>
                    <Text style={s.metaPillTxt}>{m.icon} {m.v}</Text>
                  </View>
                ))}
              </View>
            )}
          ...
          {isRestDay ? (
            <View style={s.restCard}>
              ...
              <View style={s.restTips}>
                {['Stretch for 10 minutes','Stay hydrated','Get 8 hours of sleep','Light walk is fine'].map((t,i) => (
            ...
            <>
              {selectedWorkout.exercises.map((ex, i) => {
                const info = EXERCISES.find(e => e.id === ex.id) || { name:ex.id, muscle:'', equipment:'' };
                return (
                  <View key={i} style={s.exRow}>
                    <View style={s.exNum}><Text style={s.exNumTxt}>{i+1}</Text></View>
                    <View style={s.exInfo}>
                      <Text style={s.exName}>{info.name}</Text>
                      <Text style={s.exMeta}>{info.muscle} · {info.equipment}</Text>
                    </View>
                    <View style={s.exSets}>
                      {ex.duration
                        ? <Text style={s.exSetsTxt}>{ex.duration}</Text>
                        : <Text style={s.exSetsTxt}>{ex.sets}×{ex.reps}</Text>
                      }
                      <Text style={s.exRest}>{ex.restSec > 0 ? `${ex.restSec}s rest` : ''}</Text>
                    </View>
                  </View>
                );
              })}
              {isToday && (
                <TouchableOpacity style={s.startBtn} onPress={() => navigate && navigate('WorkoutActive', { workout: selectedWorkout })} activeOpacity={0.85}>
        ...
        <View style={s.card}>
          ...
          {WORKOUT_HISTORY.map((h, i) => (
            <View key={i} style={[s.histRow, i < WORKOUT_HISTORY.length-1 && s.histRowBorder]}>
              <View style={[s.histDot, h.completed ? s.histDotDone : s.histDotMissed]} />
...
const s = StyleSheet.create({
  ...
  planChip: { backgroundColor:C.purple+'25', borderRadius:20, paddingHorizontal:12, paddingVertical:6, borderWidth:1, borderColor:C.purple+'50' },
  planChipTxt: { color:C.accent, fontSize:11, fontWeight:'700' },
  weekRow: { flexDirection:'row', gap:6, marginBottom:16 },
  dayBtn: { flex:1, alignItems:'center', gap:6, paddingVertical:10, borderRadius:14, backgroundColor:C.card, borderWidth:1, borderColor:C.border },
  ...
  workoutCardDay: { color:C.sub, fontSize:10, fontWeight:'800', letterSpacing:1, marginBottom:4 },
  workoutCardName:{ color:C.text, fontSize:20, fontWeight:'800' },
  workoutMeta:    { flexDirection:'row', flexWrap:'wrap', gap:8, marginTop:10 },
  metaPill:       { backgroundColor:C.border, borderRadius:8, paddingHorizontal:10, paddingVertical:5 },
  ...
  restTitle:   { color:C.text, fontSize:20, fontWeight:'800', marginBottom:6 },
  restSub:     { color:C.sub, fontSize:13, textAlign:'center', lineHeight:19, marginBottom:18 },
  restTips:    { width:'100%', gap:10 },
  restTipRow:  { flexDirection:'row', alignItems:'center', gap:10 },
  restTipDot:  { width:6, height:6, borderRadius:3, backgroundColor:C.purple },
  restTipTxt:  { color:C.sub, fontSize:13 },
  exRow:    { flexDirection:'row', alignItems:'center', gap:12, paddingVertical:12, borderBottomWidth:1, borderBottomColor:C.border },
  exNum:    { width:28, height:28, borderRadius:8, backgroundColor:C.border, alignItems:'center', justifyContent:'center' },
  ...
  startBtn:    { backgroundColor:C.purple, borderRadius:14, paddingVertical:16, alignItems:'center', marginTop:16 },
