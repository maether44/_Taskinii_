...
const POSTURE_HISTORY = [
  ...
  { day: 'Thu', score: 82 }, { day: 'Fri', score: 80 }, { day: 'Sat', score: 85 },
  { day: 'Sun', score: 88 },
];

const BODY_ZONES = [
  { zone: 'Neck',      status: 'Forward head posture', severity: 'medium', icon: '⚠️', tip: 'Tuck your chin and draw your head back over your shoulders.' },
  { zone: 'Shoulders', status: 'Good alignment',       severity: 'good',   icon: '✅', tip: 'Keep it up — shoulders are neutral and relaxed.' },
  { zone: 'Spine',     status: 'Slight lumbar curve',  severity: 'low',    icon: '⚠️', tip: 'Engage your core slightly when sitting for long periods.' },
  { zone: 'Hips',      status: 'Neutral position',     severity: 'good',   icon: '✅', tip: 'Hip alignment is ideal. Great foundation posture.' },
];
...
const SEV_COLOR = { good: '#34C759', medium: '#FF9500', low: '#FFD60A' };
const SEV_BG    = { good: '#34C75918', medium: '#FF950018', low: '#FFD60A18' };
...
export default function PostureAI() {
  ...
  const startScan = () => {
    setScanning(true);
    setScanned(false);
    AlexiEvents.emit('dataUpdated', { type: 'posture_scan_start' });
    Animated.loop(
    ...
    setTimeout(() => {
      ...
      AlexiEvents.emit('dataUpdated', { type: 'posture_scan_done', score: newScore });
    }, 2400);
  };
        ...
        <View style={s.cameraCard}>
          <View style={s.viewfinder}>
            {['TL', 'TR', 'BL', 'BR'].map(pos => (
              <View key={pos} style={[
                s.corner,
                pos === 'TL' && s.cornerTL, pos === 'TR' && s.cornerTR,
                pos === 'BL' && s.cornerBL, pos === 'BR' && s.cornerBR,
              ]} />
            ))}
            ...
            <View style={s.statusLabel}>
              <View style={[s.statusDot, { backgroundColor: scanning ? C.orange : scanned ? C.green : C.sub }]} />
        ...
        <View style={s.card}>
          <View style={s.cardTitleRow}>
            ...
            <Text style={[s.weekAvg, { color: scoreColor }]}>
              Avg {Math.round(POSTURE_HISTORY.reduce((a, h) => a + h.score, 0) / POSTURE_HISTORY.length)}
            </Text>
          </View>
            ...
            {POSTURE_HISTORY.map((h, i) => {
              ...
              return (
                <View key={i} style={s.barCol}>
                  ...
                  <View style={s.barBg}>
                    <View style={[s.barFill, { height: barH, backgroundColor: isToday ? col : C.border }]} />
                  </View>
          ...
          {BODY_ZONES.map((z, i) => {
            ...
            return (
              ...
              >
                ...
                <View style={[s.sevBadge, { backgroundColor: SEV_BG[z.severity] }]}>
                  <Text style={[s.sevTxt, { color: SEV_COLOR[z.severity] }]}>{z.severity}</Text>
                </View>
              </TouchableOpacity>
            );
          })}
...
function makeStyles(C) {
  return StyleSheet.create({
    ...
    aiBadge:    { backgroundColor: C.purple + '25', borderRadius: 20, paddingHorizontal: 14, paddingVertical: 7, borderWidth: 1, borderColor: C.purple + '50' },
    aiBadgeTxt: { color: C.accent, fontSize: 12, fontWeight: '700' },

    cameraCard: { backgroundColor: C.card, borderRadius: 20, padding: 18, marginBottom: 14, borderWidth: 1, borderColor: C.border, alignItems: 'center' },
    viewfinder: { width: '100%', height: 240, backgroundColor: '#080614', borderRadius: 16, alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden', marginBottom: 16 },
    ...
    scanLine:  { position: 'absolute', top: 0, left: 0, right: 0, height: 2, backgroundColor: C.lime, shadowColor: C.lime, shadowOpacity: 0.8, shadowRadius: 6 },
    scoreOverlay:  { position: 'absolute', bottom: 12, right: 12, backgroundColor: C.purple + 'DD', borderRadius: 14, padding: 10, alignItems: 'center', borderWidth: 1, borderColor: C.purple },
    overlayScore:  { fontSize: 28, fontWeight: '900' },
    overlayLabel:  { color: 'rgba(255,255,255,0.7)', fontSize: 10, fontWeight: '600' },
    statusLabel:   { position: 'absolute', bottom: 12, left: 12, flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: 10, paddingHorizontal: 10, paddingVertical: 5 },
    statusDot:     { width: 7, height: 7, borderRadius: 4 },
    statusTxt:     { color: 'rgba(255,255,255,0.8)', fontSize: 11, fontWeight: '600' },
    scanBtn:        { backgroundColor: C.purple, borderRadius: 14, paddingVertical: 14, paddingHorizontal: 40 },
    ...
    barBg:     { width: '100%', height: 80, backgroundColor: C.border, borderRadius: 6, justifyContent: 'flex-end', overflow: 'hidden' },
    ...
    exIconWrap:  { width: 38, height: 38, borderRadius: 12, backgroundColor: C.border, alignItems: 'center', justifyContent: 'center' },
    exIcon:      { fontSize: 18 },
    exInfo:      { flex: 1 },
