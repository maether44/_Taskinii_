      ...
      >
        ...
        <View style={s.cameraCard}>
          <View style={s.viewfinder}>
            ...
            {scanned && !scanning && (
              <Animated.View style={[s.scoreOverlay, { opacity: fadeAnim }]}>
                <Text style={[s.overlayScore, { color: scoreColor }]}>
                  {score}
                </Text>
          ...
          >
            <Text style={s.scanBtnTxt}>
              {scanning ? "⏳  Analyzing…" : "📷  Scan Posture"}
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={s.card}>
          <View style={s.cardTitleRow}>
            ...
            <Text style={[s.weekAvg, { color: scoreColor }]}>
              Avg{" "}
              {Math.round(
                POSTURE_HISTORY.reduce((a, h) => a + h.score, 0) /
                  POSTURE_HISTORY.length,
              )}
            </Text>
          </View>
          <View style={s.chartRow}>
            {POSTURE_HISTORY.map((h, i) => {
              ...
              const col =
                h.score >= 85 ? C.green : h.score >= 70 ? C.orange : C.red;
              return (
                <View key={i} style={s.barCol}>
                  {isToday && (
                    <Text style={[s.barValLbl, { color: col }]}>{h.score}</Text>
                  )}
                  <View style={s.barBg}>
                    <View
                      style={[
                        ...
                        {
                          height: barH,
                          backgroundColor: isToday ? col : C.border,
                        },
                      ]}
                    />
                  </View>
                  <Text style={[s.barDay, isToday && { color: C.lime }]}>
                    {h.day}
                  </Text>
                </View>
              );
          ...
          {BODY_ZONES.map((z, i) => {
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  s.zoneRow,
                  i < BODY_ZONES.length - 1 && s.zoneRowBorder,
                ]}
              ...
              >
                ...
                <View
                  style={[
                    s.sevBadge,
                    { backgroundColor: severityBg[z.severity] },
                  ]}
                >
                  <Text
                    style={[s.sevTxt, { color: severityColor[z.severity] }]}
                  >
                    {z.severity}
                  </Text>
                </View>
        ...
        <View style={s.card}>
          ...
          {EXERCISES.map((ex, i) => (
            <View
              key={i}
              style={[s.exRow, i < EXERCISES.length - 1 && s.exRowBorder]}
            >
