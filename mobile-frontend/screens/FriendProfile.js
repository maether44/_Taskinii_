...
function getAchievementIconName(achievement) {
  if (!achievement?.earned) return 'lock-closed-outline';
  return typeof achievement?.icon === 'string' && achievement.icon.trim()
    ? achievement.icon
    : 'trophy-outline';
}
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          <View style={styles.cardTitleRow}>
            <Text style={styles.achievementsCardLabel}>ACHIEVEMENTS</Text>
            <Text style={styles.achievementsCardSub}>
          ...
          <View style={styles.badgesGrid}>
            {(showAllAchievements ? achievements : achievements.slice(0, 6)).map((a) => (
              <View key={a.id} style={[styles.badge, !a.earned && styles.badgeLocked]}>
                <View style={[styles.badgeIconWrap, a.earned && styles.badgeIconWrapEarned]}>
                  <Ionicons
                    name={getAchievementIconName(a)}
                    size={22}
                    color={a.earned ? C.purple : C.sub}
                  />
                </View>
                <Text style={[styles.badgeLabel, !a.earned && styles.badgeLabelLocked]}>{a.name}</Text>
                <Text style={[styles.badgeXP, !a.earned && styles.badgeXPLocked]}>
                  +{a.xp_reward} XP
                </Text>
              </View>
            ))}
...
const styles = StyleSheet.create({
  ...
  achievementsCardSub: { color: C.sub, fontSize: 12 },
  xpRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
