  ...
  return (
    <View style={st.tierRow}>
      ...
      <View style={st.labelCol}>
        <Text style={[st.tierLabel, tier.unlocked && { color: '#fff' }]}>
          {tier.label} Report
        </Text>
        {tier.unlocked ? (
...
const st = StyleSheet.create({
  ...
  streakPillText: { color: '#A38DF2', fontSize: 11, fontWeight: '700' },
  pathContainer: { paddingLeft: 4 },
  ...
  nodeStreakNum: { color: '#7A6AAA', fontSize: 11, fontWeight: '800' },
  connector: {
    width: 3,
    height: 32,
  ...
  connectorFill: { width: '100%', borderRadius: 2 },
  labelCol: { flex: 1, paddingLeft: 12, paddingTop: 4, paddingBottom: 20 },
  tierLabel: { color: '#7A6AAA', fontSize: 13, fontWeight: '700' },
  tierStatus: { color: '#4A4268', fontSize: 10, marginTop: 2 },
})
