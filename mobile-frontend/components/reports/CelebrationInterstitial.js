...
export default function CelebrationInterstitial({ visible, milestone, onClaim, onSkip }) {
  ...
  }, [visible, meta, scaleAnim, fadeAnim, emojiScale, btnFade])

  if (!meta) return null

  const confettiDots = Array.from({ length: 30 }).map((_, index) => (
    <ConfettiDot
      key={index}
      delay={index * 80}
    ...
    >
      <View style={st.backdrop}>
        ...
        <Animated.View style={[st.card, { opacity: fadeAnim, transform: [{ scale: scaleAnim }] }]}>
          ...
          <Text style={st.desc}>
            Your consistency unlocked a personalized BodyQ performance report.
          </Text>
...
const st = StyleSheet.create({
  ...
  sub: { fontSize: 14, fontWeight: '700', textAlign: 'center', marginBottom: 16, letterSpacing: 0.5 },
  streakBadge: {
  ...
  streakText: { color: '#A38DF2', fontSize: 12, fontWeight: '700' },
  desc: { color: '#7A6AAA', fontSize: 13, lineHeight: 20, textAlign: 'center', marginBottom: 28 },
  btnGroup: { width: '100%', gap: 12 },
  claimBtn: { borderRadius: 14, paddingVertical: 14, alignItems: 'center' },
  claimText: { color: '#0F0B1E', fontSize: 16, fontWeight: '800' },
  skipBtn: { alignItems: 'center', paddingVertical: 8 },
  skipText: { color: '#7A6AAA', fontSize: 13, fontWeight: '600' },
})
