      ...
      <View style={[s.sidebar, { paddingTop: insets.top }]}>
        ...
        >
            ...
            <View style={{ paddingHorizontal: 8, gap: 6 }}>
              {[1, 2].map((i) => (
                <View key={i} style={[s.insightCard, { borderLeftColor: "#3D2F7A" }]}>
                  ...
                  />
                  <View
                    style={{ width: "90%", height: 8, borderRadius: 4, backgroundColor: "#2D2850" }}
                  />
                </View>
              ))}
            </View>
          ...
          {!insightsLoading && userInsights.length === 0 && (
            <TouchableOpacity
              style={s.insightsEmptyCard}
              onPress={refreshInsights}
              activeOpacity={0.8}
            >
              <Text style={{ fontSize: 22, marginBottom: 6 }}>✨</Text>
              <Text style={s.insightsEmptyTitle}>No insights yet</Text>
              <Text style={s.insightsEmptySubtitle}>
                Tap to generate your personalised profile insights
              </Text>
            </TouchableOpacity>
          )}

          {!insightsLoading &&
            userInsights.map((ins, i) => (
              <View key={i} style={[s.insightCard, { borderLeftColor: ins.color }]}>
                ...
                <Text style={s.insightCardMsg} numberOfLines={4}>
                  {ins.message}
                </Text>
              </View>
            ))}
