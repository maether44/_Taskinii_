      ...
      <Animated.View style={[s.card, { opacity, transform: [{ scale }] }]}>
        ...
        <View style={[s.iconCircle, isStreak && s.iconCircleStreak]}>
          {isStreak ? (
            ...
            <Ionicons name={current.icon || "trophy-outline"} size={38} color="#7C5CFC" />
          )}
        </View>

        {isStreak ? (
          <>
            <Text style={s.mainTitle}>{current.days}-Day Streak!</Text>
            <Text style={s.subTxt}>{streakMessage(current.days)}</Text>
          </>
        ) : (
          <>
            <Text style={s.mainTitle}>{current.name}</Text>
            {current.description ? <Text style={s.subTxt}>{current.description}</Text> : null}
          </>
        )}
