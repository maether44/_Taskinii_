...
const isScheduleRequest = (text) => SCHEDULE_KEYWORDS.some((k) => text.toLowerCase().includes(k));
  ...
  return (
    <Animated.View style={[spc.card, { opacity: fade, transform: [{ translateY: slide }] }]}>
      <View style={spc.header}>
        ...
        <View style={spc.aiBadge}>
          <Text style={spc.aiBadgeTxt}>✦ AI</Text>
        </View>
      </View>
        ...
        {days.slice(0, 7).map((d, i) => {
          ...
          return (
            <View key={i} style={spc.dayChip}>
              ...
              <Text
                style={[spc.dayType, { color: d.is_rest ? '#4A4268' : color }]}
                numberOfLines={1}
              >
      ...
      {days[0] && (
        <View style={spc.targets}>
          ...
          <View style={spc.targetChip}>
            <Text style={spc.targetIcon}>👟</Text>
            <Text style={spc.targetTxt}>
              {((days[0].steps_target ?? 8000) / 1000).toFixed(0)}k steps
            </Text>
          </View>
          <View style={spc.targetChip}>
            <Text style={spc.targetIcon}>💧</Text>
...
const getGreeting = (profile, name) =>
  profile
...
export default function YaraAssistant({ onOpenSchedule }) {
  ...
  const persist = async (convs) => {
    try {
      ...
      logError('YaraAssistant: persist error', e);
    }
  };
  ...
  const glowOpacityOuter = fabScale.interpolate({
    inputRange: [1, 1.08],
          ...
          {conversations.map((conv) => (
            ...
            >
              <View style={{ flex: 1, minWidth: 0 }}>
                <Text
                  style={[s.convTitle, conv.id === activeConvId && s.convTitleActive]}
                  numberOfLines={1}
                  ellipsizeMode="tail"
                >
                  {conv.title}
  ...
  return (
    <>
      ...
      {open && (
        ...
        >
          <Animated.View
            style={[s.sheet, { paddingTop: insets.top, transform: [{ translateY: slideY }] }]}
          >
            {IS_WIDE ? (
              ...
              <>
                ...
                {sidebarOpen && (
                  <>
                    <Animated.View
                      style={[StyleSheet.absoluteFill, s.sidebarBackdrop, { opacity: sidebarFade }]}
                    >
                      <TouchableOpacity
                    ...
                    <Animated.View
                      style={[s.sidebarOverlay, { transform: [{ translateX: sidebarX }] }]}
                    >
                      {renderSidebar()}
                    </Animated.View>
                  </>
...
const s = StyleSheet.create({
  ...
  voiceErrorBar: {
    ...
    paddingVertical: 6,
    borderTopWidth: 1,
    borderTopColor: 'rgba(255,80,80,0.2)',
  },
